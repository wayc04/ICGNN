...
# return the topological conflict values
def get_ics_value(args, Pi, gpr, idx_train, labels):
    ...
    gpr_matrix = gpr.float().to(device)  # class-accumulated personlized pagerank

    nnode = ppr_matrix.size(0)
    idx_unlabel = torch.LongTensor(list(set(range(gpr_matrix.size(0))) - set(idx_train.tolist()))).to(device)

    # computing the ICS values for labeled nodes
    ...
    label_matrix[idx_unlabel] = 0

    cf_matrix = torch.sum(rn_matrix * label_matrix, dim = 1)
    cf_matrix[idx_unlabel] = torch.max(rn_matrix, dim=1)[0][idx_unlabel]

    ics_list = cf_matrix.cpu().tolist()
