...
class GCN(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight, return_feature = False):
        if self.with_relu:
            feat = F.relu(self.gc1(x, edge_index,edge_weight))
        else:
            feat = self.gc1(x, edge_index,edge_weight)

        x = F.dropout(feat, self.dropout, training=self.training)
